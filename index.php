...
$app->get('/semuadosen', function() use($app, $db) {
	$dosen["error"] = false;
	$dosen["message"] = "Berhasil mendapatkan semua dosen";
...
$app->get('/dosen/{id}', function($request, $response, $args) use($app, $db) {
	$dosen = $db->tbl_dosen()->where('id', $args['id']);
	$dosendetail = $dosen->fetch();

	if ($dosen->count() == 0) {
		$responseJson["error"] = true;
		$responseJson["message"] = "Nama dosen belum tersedia";
